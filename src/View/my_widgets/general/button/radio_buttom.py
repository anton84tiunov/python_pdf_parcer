...
class MyRadioButton(QtWidgets.QToolButton):
    ...
    def __init__(self, parent: QtWidgets,  icon: os.path):
        super().__init__(parent)
        self.par: QtWidgets = parent 

        self.setMinimumSize(QtCore.QSize(30, 30))
        self.setStyleSheet(f"image: url({os.path.join(icon_dir, icon) });background-image: url({os.path.join(icon_dir, icon)});")
        self.setText("")
